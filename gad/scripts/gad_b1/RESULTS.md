...
*Source: the trap-copied Ray session logs (`logs/ray-<jobid>/`) hold the complete per-step metric stream through gs492 for both arms; the SLURM `.out` console froze mid-run (base@374, replay@212) but the Ray copy is complete. `aggregate_results.py` reads the Ray logs (deduped by step). Both arms cover all 492 steps — no window restriction. All discriminator collapses (<0.5) occur early (steps <212).*
...
- **d_acc_fresh is the full run (steps 1–492)** — recovered from the Ray session logs after the SLURM console froze mid-run; both arms cover all 492 steps, so the stability comparison is complete (not window-limited).
