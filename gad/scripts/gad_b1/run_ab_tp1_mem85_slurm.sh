...
#SBATCH --job-name=gad-ab-tp1m85
#SBATCH --output=/home/saadlahrichi/gad_run/logs/ab-tp1m85-%j.out
...
echo "===== 4/4: launch 3-step A/B smoke (variant C: TP=1, gpu_mem 0.85, no checkpoint) ====="
# From the runbook: VLLM_USE_V1=0 avoids the vllm-v1 engine breaking on 0.8.5 configs.
...
grep -aoE "timing_s/step:[0-9.]+|timing_s/gen:[0-9.]+|perf/throughput:[0-9.]+" "${SLURM_JOB_ID:+/home/saadlahrichi/gad_run/logs/ab-tp1m85-${SLURM_JOB_ID}.out}" 2>/dev/null | tail -12 || true
echo "DONE"
