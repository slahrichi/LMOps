...
#SBATCH --job-name=gad-ab-bf16d
#SBATCH --output=/home/saadlahrichi/gad_run/logs/ab-bf16d-%j.out
...
echo "===== 4/4: launch 3-step smoke (variant E: TP=1, gpu_mem 0.85, vLLM v1, bf16 DISCRIMINATOR) ====="
# OFF-PAPER EXPERIMENT: the paper/verl default trains the critic (discriminator) in fp32,
# which disables FlashAttention2 for it (log: "current dtype torch.float32", mfu/critic~6.5%).
# Here we force the critic to bf16 so FA2 engages -> should cut timing_s/update_critic.
# Uses vLLM v1 to match variant D -> E vs D isolates the bf16-discriminator effect.
# This is a deviation from the paper's fp32 discriminator.
...
grep -aoE "timing_s/step:[0-9.]+|timing_s/gen:[0-9.]+|timing_s/update_critic:[0-9.]+|perf/mfu/critic:[0-9.]+|perf/throughput:[0-9.]+" "${SLURM_JOB_ID:+/home/saadlahrichi/gad_run/logs/ab-bf16d-${SLURM_JOB_ID}.out}" 2>/dev/null | tail -16 || true
echo "DONE"
