...
#SBATCH --job-name=gad-smoke
#SBATCH --output=/home/saadlahrichi/gad_run/logs/smoke-%j.out
...
echo "===== 3/4: ensure verl on 'warmup' branch ====="
cd $VERL
git checkout warmup 2>&1 | tail -3
git log --oneline -1
echo ""

echo "===== 4/4: launch 3-step smoke ====="
...
ls -la $WORKDIR/checkpoints/smoke-warmup/ 2>&1 || echo "no checkpoint dir yet"
echo "DONE"
